...
struct Terminal {
    ...
    void resize(int c, int r) {
        ...
        if (in_alt) {
            ...
            for (int i = 0; i < cr; i++) {
                ...
                for (int j = 0; j < cc; j++) new_alt[i][j] = alt_screen[i][j];
            }
    ...
    void clear_screen(int mode) {
        if (mode == 0) { clear_line(0); for (int r = cy+1; r < rows; r++) screen[r] = std::string(cols, ' '); }
        else if (mode == 1) { for (int r = 0; r < cy; r++) screen[r] = std::string(cols, ' '); clear_line(1); }
    ...
    void handle_csi(char fin, const char* params) {
        ...
        if (esc_priv && (fin == 'h' || fin == 'l')) {
            if (p[0] == 1049 || p[0] == 47) {
                if (fin == 'h' && !in_alt) {
                    in_alt = true; alt_screen = screen; alt_cx = cx; alt_cy = cy;
                    for (int r = 0; r < rows; r++) screen[r] = std::string(cols, ' ');
                    cx = cy = 0;
                } else if (fin == 'l' && in_alt) {
                    in_alt = false; screen = alt_screen; cx = alt_cx; cy = alt_cy;
                }
            }
            return;
        }
    ...
    void feed(char c) {
        ...
        if (esc_state == 0) {
            ...
            if (c == '\n')   { cy++; if (cy >= rows) { cy = rows-1; scroll_up(); } wrap_pending = false; return; }
...
int main(int,char**) {
    ...
    s.onTouch([&](const TouchEvent& e) {
        if (e.action == TouchAction::DOWN) {
            ...
            gTouchWasBtn=false; gHeldKey=nullptr;

            // confirm exit dialog
            if (gScreen == ScreenStatus::CONFIRM_EXIT) {
                gTouchWasBtn=true;
                if (e.x>=0.10f&&e.x<=0.44f&&e.y>=0.51f&&e.y<=0.61f) {
            ...
            // keyboard key hold for repeat
            if (gKbVisible&&e.y>=gKbY0) {
                const Key* k=hitTest(e.x,e.y);
                if (k) {
    ...
    s.onFrame([&](float) {
...

        {
            ...
            for (int r=0; r<gTerm.rows; r++) {
                ...
                if (gTerm.scroll_off>0) {
                    ...
                    if (hist_idx>=0&&hist_idx<hist_sz) line_str=gTerm.history[hist_idx];
                    else if (hist_idx>=hist_sz) { int sr=hist_idx-hist_sz; if(sr<gTerm.rows) line_str=gTerm.screen[sr]; }
                } else { line_str=gTerm.screen[r]; }
                if (!line_str.empty()) Text::draw(line_str.c_str(),0.02f,y,txtS,0.72f,0.92f,0.62f);
                draw_r++;
            }
            if (gTerm.scroll_off==0) {
        ...
        if (gKbVisible) {
            ...
            for (auto it=gFlashes.begin(); it!=gFlashes.end();) {
                ...
                drawRoundRect(it->kx0,it->ky0,it->kx1,it->ky1,0.55f,0.85f,0.65f,(1.f-u)*(1.f-u)*0.55f,0.007f);
                ++it;
            }
        }
